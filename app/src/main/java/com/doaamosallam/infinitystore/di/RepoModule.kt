...
object RepoModule {
    ...
    @Provides
    @Singleton
    fun provideSearchProductRepo(apiService: APIService): ProductSearchRepo =
        ProductSearchRepoImp(apiService)

    //provide categories repo
    @Provides
    @Singleton
    fun provideCategoriesRepo(apiService: APIService): CategoriesRepo =
        CategoriesRepoImp(apiService)
}